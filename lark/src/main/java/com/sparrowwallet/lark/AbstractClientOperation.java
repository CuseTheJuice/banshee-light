...
public abstract class AbstractClientOperation implements ClientOperation {
    ...
    public boolean matches(HardwareClient client) {
        if(deviceType != null && devicePath != null) {
            return client.getType().equals(deviceType) && BansheeSerialProbe.samePortPath(client.getPath(), devicePath);
        }
        if(deviceType != null) {
        ...
        if(fingerprint != null) {
            String clientFp = client.fingerprint();
            return clientFp != null && clientFp.equals(Utils.bytesToHex(fingerprint));
        }
        return false;
    }
