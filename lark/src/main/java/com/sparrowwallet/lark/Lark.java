...
public class Lark {
    ...
    /**
     * List connected Banshee devices without opening the serial port or reading fingerprints.
     * Used for background USB status polling.
     */
    public List<HardwareClient> enumerateWithoutFingerprint() {
    ...
    private void enumerate(ClientOperation clientOperation) throws DeviceException {
        synchronized(lock) {
            try {
                ...
                log.error("HID enumerate failed; still scanning Banshee serial", e);
            }
            enumerateBansheeSerialClients(clientOperation);
    ...
    private void enumerateBansheeSerialClients(ClientOperation clientOperation) throws DeviceException {
        if(!clientOperation.requires(Interface.SERIAL)) {
            return;
        }

        boolean allowProbe = !(clientOperation instanceof EnumerateOperation)
                || (clientOperation instanceof InitializeFingerprintOperation);
        Set<HardwareClient> foundClients = new LinkedHashSet<>();
        for(SerialPort serialPort : SerialPort.getCommPorts()) {
            if(!BansheeSerialProbe.matches(serialPort, allowProbe)) {
                continue;
            }

            HardwareClient hardwareClient = null;
            try {
                hardwareClient = new BansheeClient(serialPort);
        ...
        if(!foundClients.isEmpty() && clientOperation.success()) {
            boolean usbFailed = foundClients.stream().allMatch(c -> c.error() != null);
            if(!usbFailed) {
                return;
            }
        }
        boolean listBle = BansheeBle.supported() && (BansheeBle.hasTarget() || BansheeBle.alive());
        if(listBle) {
            HardwareClient bleClient = new BansheeClient(BansheeBle.pathOf(BansheeBle.selected()));
            bleClient.setWalletNames(walletNames);
            try {
                if(clientOperation != null && clientOperation.matches(bleClient) && foundClients.add(bleClient)) {
                    clientOperation.apply(bleClient);
                }
            } catch(DeviceException e) {
                if(clientOperation instanceof InitializeFingerprintOperation) {
                    bleClient.setError("Could not open Banshee over Bluetooth: " + e.getMessage());
                    foundClients.add(bleClient);
                } else if(!(e instanceof DeviceNotFoundException)) {
                    throw e;
                }
            }
        }
        if(!clientOperation.success() && foundClients.size() == 1
                && (clientOperation instanceof SignPsbtOperation || clientOperation instanceof GetBansheeEntropyOperation)) {
            clientOperation.apply(foundClients.iterator().next());
        }
    }
        ...
        try {
            ...
            for(HidDevice hidDevice : hidServices.getAttachedHidDevices()) {
                ...
                try {
                    ...
                    if(hardwareClient.getHardwareType() != HardwareType.LEDGER) {
                        continue;
                    }
                    hardwareClient.setWalletNames(walletNames);
    ...
    /**
     * Reads Banshee's hash-only entropy receipt and a fresh RNG health sample.
     * Serial I/O stays in this module (jSerialComm is not on the sparrow classpath).
     */
    public BansheeEntropyReport getBansheeEntropyProof(String devicePath) throws DeviceException {
        GetBansheeEntropyOperation operation = new GetBansheeEntropyOperation(devicePath);
        enumerate(operation);
        return operation.getReport();
    }

    public <T> T withBanshee(String devicePath, ApplyBansheeOperation.ClientFn fn) throws DeviceException {
        ApplyBansheeOperation operation = new ApplyBansheeOperation(devicePath, fn);
        enumerate(operation);
        return operation.getResult();
    }

    public BansheeClient.BansheeDiceResult generateBansheeDiceWallet(String devicePath, java.util.function.Consumer<String> progress) throws DeviceException {
        return withBanshee(devicePath, client -> client.generateDiceWallet(progress));
    }
