...
public class Config {
    ...
    private boolean checkNewVersions = false;
    private Theme theme;
    ...
    private String bansheeStudioUrl;
    private String bansheeOracleUrl;
    private String bansheeStudioToken;
    private String bansheeStudioFingerprint;
    private boolean bansheeStudioSigning;
    private String bansheeInstalledVersion;
    private String bansheeBleAddress;
    ...
    public String getBansheeStudioUrl() {
        if(bansheeStudioUrl == null || bansheeStudioUrl.isBlank()) {
            return "https://localhost:8457";
        }
        return bansheeStudioUrl.trim();
    }

    public void setBansheeStudioUrl(String bansheeStudioUrl) {
        this.bansheeStudioUrl = bansheeStudioUrl == null ? null : bansheeStudioUrl.trim();
        flush();
    }

    public String getBansheeOracleUrl() {
        if(bansheeOracleUrl == null || bansheeOracleUrl.isBlank()) {
            return getBansheeStudioUrl();
        }
        return bansheeOracleUrl.trim().replaceAll("/+$", "");
    }

    public void setBansheeOracleUrl(String bansheeOracleUrl) {
        this.bansheeOracleUrl = bansheeOracleUrl == null ? null : bansheeOracleUrl.trim();
        flush();
    }

    public String getBansheeStudioToken() {
        return bansheeStudioToken == null ? "" : bansheeStudioToken.trim();
    }

    public void setBansheeStudioToken(String bansheeStudioToken) {
        this.bansheeStudioToken = bansheeStudioToken == null ? null : bansheeStudioToken.trim();
        flush();
    }

    public boolean hasBansheeStudioLink() {
        return !getBansheeStudioToken().isEmpty();
    }

    /**
     * Off by default: Light signs over USB or Bluetooth. Only when this is on
     * does a link code route signing via Studio.
     */
    public boolean isBansheeStudioSigning() {
        return bansheeStudioSigning;
    }

    public void setBansheeStudioSigning(boolean bansheeStudioSigning) {
        if(this.bansheeStudioSigning == bansheeStudioSigning) {
            return;
        }
        this.bansheeStudioSigning = bansheeStudioSigning;
        flush();
    }

    /** Last fingerprint Studio reported, so a linked wallet still matches while Studio is offline. */
    public String getBansheeStudioFingerprint() {
        return bansheeStudioFingerprint == null ? "" : bansheeStudioFingerprint.trim();
    }

    public void setBansheeStudioFingerprint(String bansheeStudioFingerprint) {
        if(bansheeStudioFingerprint != null && bansheeStudioFingerprint.equals(this.bansheeStudioFingerprint)) {
            return;
        }
        this.bansheeStudioFingerprint = bansheeStudioFingerprint == null ? null : bansheeStudioFingerprint.trim();
        flush();
    }

    public String getBansheeBleAddress() {
        return bansheeBleAddress == null ? "" : bansheeBleAddress.trim();
    }

    public void setBansheeBleAddress(String bansheeBleAddress) {
        this.bansheeBleAddress = bansheeBleAddress == null ? null : bansheeBleAddress.trim();
        flush();
    }

    public String getBansheeInstalledVersion() {
        return bansheeInstalledVersion == null ? "" : bansheeInstalledVersion;
    }

    public void setBansheeInstalledVersion(String bansheeInstalledVersion) {
        this.bansheeInstalledVersion = bansheeInstalledVersion;
        flush();
    }
