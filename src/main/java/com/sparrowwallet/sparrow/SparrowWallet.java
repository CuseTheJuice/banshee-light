...
public class SparrowWallet {
    public static final String APP_ID = "banshee-light";
    public static final String APP_NAME = "Banshee Light";
    ...
    private static void allowBansheeOracle() {
        try {
            java.io.File dir = new java.io.File(Storage.getConfigHome(), "oracle");
            com.sparrowwallet.lark.BansheeOracle.useDirectory(dir);
            com.sparrowwallet.lark.BansheeBle.useDirectory(new java.io.File(Storage.getConfigHome(), "native"));
            com.sparrowwallet.lark.BansheeOracle.listenLoopback();
        } catch(Exception e) {
            getLogger().warn("Could not open the Banshee Light oracle store", e);
        }
    }

    /**
     * A link code no longer steers device detection on its own, so it is kept across upgrades.
     * Signing only goes through Studio when Sign via Studio is switched on in Preferences.
     */
    private static void clearStudioLinkOnUpgrade() {
        try {
            Config config = Config.get();
            if(APP_VERSION.equals(config.getBansheeInstalledVersion())) {
                return;
            }
            config.setBansheeInstalledVersion(APP_VERSION);
        } catch(Exception e) {
            getLogger().warn("Could not record the installed Banshee version", e);
        }
    }
