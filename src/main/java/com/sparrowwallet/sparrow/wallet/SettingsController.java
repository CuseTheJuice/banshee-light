...
public class SettingsController extends WalletFormController implements Initializable {
    ...
    private boolean pendingMultisigBackup;
    ...
    public void initializeView() {
        ...
        apply.setOnAction(event -> {
            try {
                walletForm.getWallet().checkWallet();
            } catch(InvalidWalletException e) {
                AppServices.showErrorDialog("Wallet not ready to save", e.getMessage()
                        + "\n\nEach cosigner needs a fingerprint, a derivation path, and an xpub."
                        + " Use Import on a keystore tab for a connected Banshee,"
                        + " or the eye (xpub) tile to paste one.");
                return;
            }
            ...
            pendingMultisigBackup = wallet.getPolicyType() == PolicyType.MULTI_HD
                    && wallet.getDefaultPolicy() != null
                    && wallet.getDefaultPolicy().getNumSignaturesRequired() < wallet.getKeystores().size()
                    && addressChange;
            try {
                saveWallet(false, false);
            } catch(Exception e) {
                log.error("Error applying wallet", e);
                AppServices.showErrorDialog("Error saving wallet", e.getMessage() == null ? e.toString() : e.getMessage());
                pendingMultisigBackup = false;
    ...
    private void setFieldsFromWallet(Wallet wallet) {
        if(wallet.getPolicyType() == null) {
            ...
            wallet.setDefaultPolicy(Policy.getPolicy(wallet.getPolicyType(), wallet.getScriptType(), wallet.getKeystores(), 2));
        }
        ...
        if(wallet.getPolicyType().equals(PolicyType.SINGLE_HD) || wallet.getPolicyType().equals(PolicyType.SINGLE_SP)) {
            ...
            boolean emptyKeys = wallet.getKeystores().stream().allMatch(keystore -> keystore.getExtendedPublicKey() == null);
            int cosigners = emptyKeys ? Math.max(3, wallet.getKeystores().size()) : wallet.getKeystores().size();
            int threshold = emptyKeys ? 2 : wallet.getDefaultPolicy().getNumSignaturesRequired();
            multisigControl.setMax(Math.max(Math.min(maxCosigners, multisigControl.getMax()), cosigners));
            multisigControl.highValueProperty().set(cosigners);
            multisigControl.lowValueProperty().set(threshold);
            totalKeystores.bind(multisigControl.highValueProperty());
        }

        if(wallet.getPolicyType() != null && wallet.getPolicyType() != PolicyType.MULTI_HD
                && !policyType.getItems().contains(wallet.getPolicyType())) {
        ...
        if(text.isPresent() && !text.get().isEmpty() && !text.get().equals(outputDescriptorString)) {
            ...
            while(matcher.find()) {
                ...
                if(ExtendedKey.Header.getHeaders(Network.get()).stream().anyMatch(header -> header.isPrivateKey() && extKey.startsWith(header.name())) &&
                    ...
                    AppServices.showWarningDialog("Private extended key detected", "Banshee Light will convert the provided private key to a public key for use in a watch only wallet.\n\nUse Sparrow to import a software seed or master private key.");
    ...
    private void walletSaved() {
        if(!pendingMultisigBackup) {
            return;
        }
        pendingMultisigBackup = false;
        Platform.runLater(this::showMultisigBackup);
    }

    private void showMultisigBackup() {
        try {
            Wallet wallet = walletForm.getWallet();
            ...
            try {
                ...
                if(registryItem != null) {
                    ur = registryItem.toUR();
                }
            } catch(Exception e) {
                log.warn("Could not encode descriptor UR", e);
            }
            MultisigBackupDialog dialog = new MultisigBackupDialog(wallet, outputDescriptor, ur);
            dialog.initOwner(apply.getScene().getWindow());
            dialog.showAndWait();
        } catch(Exception e) {
            log.error("Error showing multisig backup", e);
            AppServices.showErrorDialog("Wallet saved", "The wallet was saved. Could not show the descriptor backup: "
