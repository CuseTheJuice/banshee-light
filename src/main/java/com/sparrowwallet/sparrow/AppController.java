...
public class AppController implements Initializable {
    ...
    public void showLogFile(ActionEvent event) {
        File logFile = new File(Storage.getStateHome(), "banshee.log");
        try {
            if(!logFile.exists()) {
            ...
            AppServices.get().getApplication().getHostServices().showDocument(logFile.toURI().toString());
        } catch(Exception e) {
            AppServices.showErrorDialog("Could not open log file", logFile.getAbsolutePath() + "\n" + e.getMessage());
        }
    }
        ...
        if(optionalTxId.isPresent()) {
            ...
            transactionReferenceService.setOnFailed(failEvent -> {
                Platform.runLater(() -> {
                    ...
                    showErrorDialog("Error fetching transaction", "The server returned an error when fetching the transaction. The server response is contained in banshee.log");
                });
            });
    ...
    public void restartInHome(ActionEvent event) {
        ...
        directoryChooser.setTitle("Choose Banshee Light Home Folder");
    ...
    public void setupBanshee(ActionEvent event) {
        BansheeSetupDialog dlg = new BansheeSetupDialog();
        dlg.initOwner(rootStack.getScene().getWindow());
        Object result = dlg.showAndWait().orElse(null);
    ...
    public void newWallet(ActionEvent event) {
        ...
        if(optNameAndBirthDate.isPresent()) {
            ...
            if(nameAndBirthDate.isLight()) {
                ...
                Wallet wallet = new Wallet(nameAndBirthDate.getName(), PolicyType.MULTI_HD, ScriptType.P2WSH, nameAndBirthDate.getBirthDate());
        ...
        if(parentDirs.size() == 1 && !Boolean.FALSE.equals(Config.get().getSuggestChangeWalletsDir())) {
            ...
            if(!sameDir) {
                ConfirmationAlert alert = new ConfirmationAlert("Change wallets directory?",
                    "Do you want to configure Banshee Light to use " + selectedDir + " as the default wallets directory?", ButtonType.NO, ButtonType.YES);
    ...
    public void requestVerifyDownloadOpen(RequestVerifyDownloadEvent event) {
        // Verify Download was removed from Banshee.
    }
