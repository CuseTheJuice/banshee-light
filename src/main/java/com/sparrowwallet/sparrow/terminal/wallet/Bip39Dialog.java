...
public class Bip39Dialog extends NewWalletDialog {
    ...
    public Bip39Dialog(String walletName) {
        ...
        for(PolicyType policyType : List.of(PolicyType.MULTI_HD)) {
        ...
        scriptType.setSelectedItem(new PolicyAndScriptType(PolicyType.MULTI_HD, ScriptType.P2WSH));

        seedWords.setTextChangeListener((newText, changedByUserInteraction) -> {
            try {
                String[] words = newText.split("[ \n]");
                importer.getKeystore(PolicyType.MULTI_HD, scriptType.getSelectedItem().scriptType().getDefaultDerivation(), Arrays.asList(words), passphrase.getText());
