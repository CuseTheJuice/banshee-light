...
public class DialogImage extends StackPane {
    ...
    protected void refresh(Type type) {
        String name = assetName(type);
        SVGImage svgImage;
        if(Config.get().getTheme() == Theme.DARK) {
            svgImage = loadSVGImage("/image/dialog/" + name + "-invert.svg");
        } else {
            svgImage = loadSVGImage("/image/dialog/" + name + ".svg");
        }
    ...
    /** Type.SPARROW is the "this app" mark, which for Banshee Light is the Banshee logo. */
    private static String assetName(Type type) {
        return type == Type.SPARROW ? "banshee" : type.name().toLowerCase(Locale.ROOT);
    }
