...
public class WalletNameDialog extends Dialog<WalletNameDialog.NameAndBirthDate> {
    ...
    public WalletNameDialog() {
        this("", false, null, null, false, true);
    }
    ...
    public WalletNameDialog(String initialName, boolean hasExistingTransactions, PolicyType walletPolicyType, Date startDate, boolean rename) {
        this(initialName, hasExistingTransactions, walletPolicyType, startDate, rename, false);
    }

    public WalletNameDialog(String initialName, boolean hasExistingTransactions, PolicyType walletPolicyType, Date startDate, boolean rename, boolean chooseKind) {
        ...
        dialogPane.setPrefHeight(requestBirthDate || chooseKind ? 280 : 200);
        ...
        RadioButton lightRadio = new RadioButton("Light — one Banshee over USB or Bluetooth (P2WPKH)");
        RadioButton vaultRadio = new RadioButton("Vault — 2-of-3 (Banshee + two other keys)");
        lightRadio.setToggleGroup(kindGroup);
        vaultRadio.setToggleGroup(kindGroup);
        lightRadio.setSelected(true);
        if(chooseKind) {
            content.getChildren().addAll(lightRadio, vaultRadio);
        }
        ...
        setResultConverter(dialogButton -> dialogButton == okButtonType
                ? new NameAndBirthDate(name.getText().trim(), existingPicker.getValue(), !chooseKind || lightRadio.isSelected())
                : null);
    }
    ...
    public static class NameAndBirthDate {
        ...
        public NameAndBirthDate(String name, LocalDate birthLocalDate) {
            this(name, birthLocalDate, true);
        }

        public NameAndBirthDate(String name, LocalDate birthLocalDate, boolean light) {
            this.name = name;
