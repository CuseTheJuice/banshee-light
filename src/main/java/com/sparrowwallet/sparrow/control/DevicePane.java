...
public class DevicePane extends TitledDescriptionPane {
    ...
    private Button entropyProofButton;
    ...
    private void setDefaultStatus() {
        if(device.getModel() == WalletModel.BANSHEE && deviceOperation == DeviceOperation.SIGN) {
            setDescription(StudioSignClient.usesStudio(device) ? "Confirm in Studio." : "Confirm on device.");
            return;
        }
        if(device.getModel() == WalletModel.BANSHEE) {
            setDescription("Unlock on device if locked.");
            return;
        }
    ...
    private Node getTogglePassphraseOn() {
        ...
        togglePassphraseOn.setOnAction(event -> {
            setExpanded(false);
            hideButtons(importButton, signButton, displayAddressButton, signMessageButton, entropyProofButton);
    ...
    private void sign() {
        if(device.isCard()) {
            ...
            signPSBTService.messageProperty().addListener((observable, oldValue, newValue) -> {
                if(newValue != null && !newValue.isBlank()) {
                    Platform.runLater(() -> setDescription(newValue));
                }
            });
            setDescription(StudioSignClient.usesStudio(device)
                    ? "Confirm in Studio."
                    : "Signing...");
            showHideLink.setVisible(false);
    ...
    private void maybeAddEntropyProofButton() {
        if(device.getModel() != WalletModel.BANSHEE || entropyProofButton != null
                || StudioSignClient.STUDIO_PATH.equals(device.getPath())) {
            return;
        }
        entropyProofButton = new Button("Entropy proof");
        entropyProofButton.setAlignment(Pos.CENTER_RIGHT);
        entropyProofButton.setOnAction(event -> {
            entropyProofButton.setDisable(true);
            Hwi.BansheeEntropyProofService service = new Hwi.BansheeEntropyProofService(device);
            service.setOnSucceeded(ok -> {
                showEntropyProofDialog(service.getValue());
                entropyProofButton.setDisable(false);
            });
            service.setOnFailed(fail -> {
                setError("Entropy proof", service.getException().getMessage());
                entropyProofButton.setDisable(false);
            });
            setDescription("Reading Banshee Wallet entropy receipt…");
            service.start();
        });
        buttonBox.getChildren().add(0, entropyProofButton);
    }

    private void showEntropyProofDialog(Hwi.BansheeEntropyReport report) {
        setDefaultStatus();
        Alert alert = new Alert(Alert.AlertType.INFORMATION);
        alert.setTitle("Banshee Wallet entropy proof");
        alert.setHeaderText("What this can and cannot prove");
        TextArea area = new TextArea(formatEntropyProof(report));
    ...
    private static String formatEntropyProof(Hwi.BansheeEntropyReport report) {
        Map<String, String> proof = parseGateFields(report.proof);
        Map<String, String> rng = parseGateFields(report.rng);
        String source = proof.getOrDefault("source", "unknown");
        StringBuilder out = new StringBuilder();
        out.append("A hash cannot prove the bits were random. It only binds this Banshee Wallet to a value hashed at wallet creation.\n\n");
        out.append("The seed, mnemonic, and dice transcript never leave the chip. This PC cannot display the private key.\n\n");
        out.append("Birth receipt\n");
        if("dice".equals(source) || "dice-pass".equals(source)) {
            out.append("Source: 100 on-device d6 rolls. Dice entropy is the BIP39 passphrase (25th word); chip RNG makes the hidden 24-word seed.\n");
            out.append("Dice words: ").append(proof.getOrDefault("n", "?")).append('\n');
            out.append("Bits collected: ").append(proof.getOrDefault("bits", "?")).append('\n');
            out.append("Bits used for passphrase: ").append(proof.getOrDefault("used", "?")).append('\n');
            out.append("Entropy SHA-256: ").append(proof.getOrDefault("esha", "(none)")).append('\n');
            if(proof.containsKey("tsha")) {
                out.append("Transcript SHA-256: ").append(proof.get("tsha")).append('\n');
            }
        } else if("rng".equals(source)) {
            out.append("Source: ESP32 hardware RNG (esp_random), 256 bits, no dice ceremony.\n");
            out.append("Entropy SHA-256: ").append(proof.getOrDefault("esha", "(none)")).append('\n');
        } else if("clone".equals(source)) {
            out.append("Source: cloned from another Banshee Wallet. This board did not roll the original entropy.\n");
            out.append("Seed SHA-256: ").append(proof.getOrDefault("esha", "(none)")).append('\n');
        } else {
            out.append("No receipt stored (wallet created before firmware 0.1.9, or NVS has no hash).\n");
        }
        if(proof.containsKey("fp")) {
            out.append("Master fingerprint: ").append(proof.get("fp")).append('\n');
        }
        out.append("\nLive RNG check (fresh unused bytes, not the wallet key)\n");
        out.append("Sample: ").append(rng.getOrDefault("n", "?")).append(" bytes\n");
        out.append("Unique byte values: ").append(rng.getOrDefault("unique", "?")).append(" / 256\n");
        out.append("Sample SHA-256: ").append(rng.getOrDefault("sha", "(none)")).append('\n');
        out.append("\nWatching the Banshee Wallet dice at creation is the real audit. A later hash is only a receipt.");
        return out.toString();
    }

    private static Map<String, String> parseGateFields(String line) {
