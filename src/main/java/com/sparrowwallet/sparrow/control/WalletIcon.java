    ...
    public void refresh() {
        ...
        if(wallet.getWalletConfig() != null && wallet.getWalletConfig().getIconData() != null) {
            ...
            if(keystore.getSource() == KeystoreSource.HW_USB || keystore.getSource() == KeystoreSource.HW_AIRGAPPED) {
                ...
                String type = walletModel.getType();
                boolean dark = Config.get().getTheme() == Theme.DARK;
                ImageView raster = loadRasterImage("/image/walletmodel/" + type + (dark ? "-icon-invert.png" : "-icon.png"));
                if(raster == null && dark) {
                    raster = loadRasterImage("/image/walletmodel/" + type + "-icon.png");
                }
                if(raster != null) {
                ...
                SVGImage svgImage;
                if(dark) {
                    svgImage = loadSVGImage("/image/walletmodel/" + type + "-icon-invert.svg");
                } else {
                    svgImage = loadSVGImage("/image/walletmodel/" + type + "-icon.svg");
                }
