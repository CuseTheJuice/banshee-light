...
public class WalletExportDialog extends Dialog<Wallet> {
    ...
    public WalletExportDialog(WalletForm selectedWalletForm, List<WalletForm> allWalletForms) {
        ...
        if(wallet.getPolicyType() == PolicyType.SINGLE_HD) {
            exporters = List.of(new Descriptor(), new Electrum(), new Sparrow(), new WalletLabels(allWalletForms), new WalletTransactions(selectedWalletForm));
        } else if(wallet.getPolicyType() == PolicyType.MULTI_HD) {
            exporters = List.of(new Bip129(), new Descriptor(), new Electrum(), new Sparrow(), new WalletLabels(allWalletForms), new WalletTransactions(selectedWalletForm));
