...
public class KeystoreImportController implements Initializable {
    ...
    public void initializeView(Wallet wallet) {
        this.wallet = wallet;
        Set<KeystoreSource> supported = new HashSet<>(KeystoreImportDialog.getSupportedSources());
        for(Toggle toggle : importMenu.getToggles()) {
            KeystoreSource source = (KeystoreSource) toggle.getUserData();
            boolean show = supported.contains(source);
