...
public class AppServices {
    ...
    private void restartServices() {
        ...
        // Banshee does not poll sparrowwallet.com for Sparrow releases.
    ...
    public void stop() {
        ...
        if(httpClientService != null) {
            try {
                ...
                log.warn("Could not stop HTTP clients", e);
            }
        }
        ...
        try {
            ...
            log.warn("Could not stop Light oracle", e);
        }
        try {
            BansheeBle.invalidate();
        } catch(Exception e) {
            log.warn("Could not stop Bluetooth helper", e);
        }
        killChildProcesses();
    }
    ...
    private static Image getWindowIcon() {
        if(windowIcon == null) {
            windowIcon = new Image(SparrowWallet.class.getResourceAsStream("/image/banshee-icon.png"));
        }
    ...
    public void openWallets(OpenWalletsEvent event) {
        ...
        for(WalletTabData walletTabData : allWallets) {
            ...
            if(Interface.get() == Interface.DESKTOP && wallet.containsSource(KeystoreSource.HW_USB)) {
                usbWallet = true;
